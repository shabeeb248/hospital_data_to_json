...
def validate_date_final(df):
    ...
    for index, row in df.iterrows():
        try:
            # Attempt to parse the date value using the specified format
            date = pd.to_datetime(row['DATE'])
            df.at[index, 'DATE'] = date.strftime('%Y-%m-%dT00:00:00.000Z')
            df.at[index, 'DATE-VALIDATE'] = True
        except ValueError:
            # If parsing fails, append False to the validation list
            df.at[index, 'DATE-VALIDATE'] = False
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            if len(shift_value) != 9:
                ...
                shift_start_time= datetime.strptime(f"{shift_start}00", "%H%M%S").time()
                shift_start_time= shift_start_time.strftime("%H.%M.%S.000Z")
                df.at[index, 'SHIFT START'] = shift_start_time
                print(shift_start_time)
                shift_end_time= datetime.strptime(f"{shift_end}00", "%H%M%S").time()
                shift_end_time= shift_end_time.strftime("%H.%M.%S.000Z")
                df.at[index, 'SHIFT END'] = shift_end_time
